...
read_clauses(Stream, Acc, Clauses) :-
    read_term(Stream, Term, [variable_names(VNs),module(starlog_to_prolog_cli)]),
	    numbervars(Term, 0, _),
    (   Term == end_of_file
    ->  reverse(Acc, Clauses)
...
starlog_to_pl_with_decompression(((Head :- Body),VNs), ((Head :- PBody),VNs)) :- 
    !, 
    decompress_goal(Body, VNs,PBody).%term_to_atom(PBody,PBody1).
...
write_clauses(Stream, [Clause|Clauses]) :-
    ( Clause = ((Head :- true),VNs) ->
        write_term(Stream, Head, [variable_names(VNs),quoted(true), numbervars(true)]), write(Stream, '.\n')
    ; Clause = ((Head :- Body),VNs) ->
        write_term(Stream, (Head :- Body), [variable_names(VNs),quoted(true), numbervars(true)]), write(Stream, '.\n')
    ;   write_term(Stream, Clause, [variable_names(_VNs),quoted(true), numbervars(true)]), write(Stream, '.\n')
    ),
    write_clauses(Stream, Clauses).
