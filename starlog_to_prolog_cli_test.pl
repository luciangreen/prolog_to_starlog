...
% 
% IMPORTANT: We do NOT expand :, •, or & operators when they appear as
% arguments to builtin functions. They should only be expanded at the top
% level of an 'is' expression. This prevents errors like:
%   wrap(a:a) becoming string_concat(a,a,_G), wrap(_G,...)
% instead of the correct: wrap(a:a, ...)
compile_value(Expr, Value, Goals) :-
    ( is_value_builtin_expr(Expr) ->
        % This is a builtin function call, expand it
        fresh_var(Value),
        compile_expr(Expr, Value, Goals)
    ; is_arith_expr(Expr) ->
        fresh_var(Value),
        Goals = [Value is Expr]
    ;   % Default: treat as data/leaf (including compound terms like a:a)
        Value = Expr,
        Goals = []
    ).

% Check if expression is a builtin function (not :, •, or & operators)
is_value_builtin_expr(Expr) :-
    compound(Expr),
    functor(Expr, F, A),
    F \= ':',
