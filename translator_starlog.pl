...
parse_body((A,B),C):-!,parse_body(A,D),parse_body(B,E),C is &(D,E).
parse_body(findall(A,B,C),[findall(A,B,C)]):-!.
parse_body(A,[goal(A)]).
analyze_findalls(A,B,C):-separate_components(A,D,C),B is maplist(parse_findall_structure,D).
separate_components([],[],[]).
...
generate_base_fact(A,B):-get_base_predicate_name(A,C),D is •(C,s),format(atom(B),'% ~w([...]).  % TODO: Fill in your base facts',[D]).
...
generate_main_predicate(A,B,C):-get_base_predicate_name(B,D),E is •(D,s),capitalize_atom(E,F),count_levels(B,G),H is G+1,generate_var_sequence(F,H,I),I=[J|K],last(I,L),format(atom(M),'    ~w(~w)',[E,J]),generate_chain_calls(K,1,J,N),O is &([M],N),atomics_to_string(O,',\n',P),format(atom(C),'~w(~w) :-\n~w.',[A,L,P]).
count_levels(base(A,B,C),1).
count_levels(iteration(A,B),1).
count_levels(nested(A,B,C),D):-count_levels(A,E),D is E+1.
generate_var_sequence(A,B,C):-C is findall(D,(between(1,B,E),format(atom(D),'~w~d',[A,E]))).
...
generate_helper_predicates(nested(A,B,C),D,E):-generate_helper_predicates(A,D,F),count_levels(A,G),H is D+G,format(atom(I),'findall~|~`0t~d~3+',[H]),format(atom(J),'~w([],[]).',[I]),generate_outer_recursive_clause(I,B,K),E is &(F,[J,K]).
...
generate_base_facts(A,B):-E is findall(C,(member(D,A),get_base_predicate_name(D,C),C\=none)),list_to_set(E,F),B is maplist(generate_single_base_fact,F).
generate_single_base_fact(A,B):-C is •(A,s),format(atom(B),'% ~w([...]).  % TODO: Fill in your base facts',[C]).
...
generate_main_predicate_complex(A,B,C,D,E):-H is findall(F,(member(G,C),get_base_predicate_name(G,F),F\=none)),list_to_set(H,I),N is findall(J,(member(K,I),atom_concat(K,s,L),capitalize_atom(L,M),format(atom(J),'    ~w(~w1)',[L,M]))),generate_all_chains(C,1,O),P is maplist(format_goal_call,D),append([N,O,P],Q),flatten(Q,R),(B=[S]->format(atom(T),'~w(~w)',[A,S]);format_args(B,U),format(atom(T),'~w(~w)',[A,U])),(R=[]->format(atom(E),'~w.',[T]);atomics_to_string(R,',\n',V),format(atom(E),'~w :-\n~w.',[T,V])).
format_goal_call(A,B):-format(atom(B),'    ~w',[A]).
format_args(A,B):-C is maplist(format_single_arg,A),atomics_to_string(C,', ',B).
...
generate_all_helper_predicates([A|B],C,D):-generate_helper_predicates(A,C,E),count_levels(A,F),G is C+F,generate_all_helper_predicates(B,G,H),D is &(E,H).
format_output_v2(A,B,C,D):-atomics_to_string(A,'\n',E),atomics_to_string(C,'\n',F),format(atom(D),'~w~n~n~w~n~w',[E,B,F]).
test_simple:-format('~n=== Test 1: Simple Single-Level Findall ===~n'),A='test(R) :- findall(f(X), base(X), R).',format('Input:  ~w~n~n',[A]),translate(A,B),format('Output:~n~w~n',[B]),format('✓ Test 1 completed~n').
test_with_transform:-format('~n=== Test 2: Single Findall with Transform ===~n'),A='test(R) :- findall(Y1, (colour(Y), Y1 = c-Y), R).',format('Input:  ~w~n~n',[A]),translate(A,B),format('Output:~n~w~n',[B]),format('✓ Test 2 completed~n').
test_nested:-format('~n=== Test 3: Nested Findall (Problem Statement) ===~n'),A='predicate(YYs) :- findall([Y2,Y2], (findall(Y1, (colour(Y), Y1 = c-Y), Ys), member(Y2, Ys)), YYs).',format('Input:~n'),format('predicate(YYs) :-~n'),format('    findall([Y2,Y2],~n'),format('        (findall(Y1,(colour(Y),Y1=c-Y),Ys),~n'),format('         member(Y2,Ys)),~n'),format('        YYs).~n~n'),translate(A,B),format('Output:~n~w~n',[B]),format('✓ Test 3 completed~n').
run_tests:-format('~n================================================~n'),format('  Findall to Predicates Translator~n'),format('================================================~n'),catch(test_simple,A,format('Test 1 error: ~w~n',[A])),catch(test_with_transform,A,format('Test 2 error: ~w~n',[A])),catch(test_nested,A,format('Test 3 error: ~w~n',[A])),format('~n================================================~n'),format('  All Tests Completed!~n'),format('================================================~n~n').
